...
## 1. Focused box's ring doesn't erase when focus moves to another box — fixed, uncommitted
...
**For the Janus session:** review the diff (`git diff` in the Janus
checkout — nothing is committed there yet), commit it under whatever
commit-message convention that repo's `git log` establishes, and confirm
the "not yet visually confirmed" point above however that project
verifies real-hardware behavior. `architecture.md`'s Stage 4 "Focus state"
section and Stage 6 docs will need a line added once this lands, same
pattern as the PROGMEM handoff's fix being folded into Stage 4's docs.
