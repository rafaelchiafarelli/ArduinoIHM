...
replaced by UI generated by [Janus](../../janus) from
`lib/GUI/*.screen.yaml`/`app.yaml`: `*_screen.gen.c`/`.h` (per-tab widget
trees, `PROGMEM`-resident), `janus_bindings.gen.h` (bound C structs),
`janus_actions.gen.h`, plus the fixed, hand-written runtime
(`janus_runtime.c`/`.h`, `janus_font.c`/`.h`, `janus_input_focus.c`/`.h`).
`main.cpp` supplies the driver contract (`display_driver_init`,
`draw_area_sync`/`_async`, `display_busy`) on top of the existing
`Display`/mcufriend-style parallel-TFT driver, and drives screen-switch
(rot0) / focus-move+activate (rot1) input. This migration itself predates
this session (done, uncommitted, in an earlier untracked working-tree
state) -- this session is its first real hardware bring-up and first
commit.

First boot surfaced two real bugs, both the same root mistake, both in
Janus's *fixed runtime library* (not the generated `.gen.c` files, not
this project's glue) -- see `../../handle-to-janus.md` (repo root) for the
full writeup meant for porting back into the Janus project itself:

1. `janus_app_t.screens` is `JANUS_PROGMEM` (an array of pointers living
   in flash) -- `janus_app.screens[index]`/`app->screens[index]` (5 spots
   in `main.cpp`, plus `janus_switch_screen()`/
   `janus_switch_screen_async_start()` in `janus_runtime.c`) read it with
   plain indexing, an ordinary AVR load from what's actually a flash byte
   offset -- garbage, not the real screen pointer.
2. One level deeper: every generated `janus_screen_desc_t` instance
   (`pwm_screen`, etc.) is *itself* `JANUS_PROGMEM`. `janus_render_screen()`
   and `janus_input_focus.c`'s `walk_screen()` read
   `screen->widget_count`/`screen->widgets`/`screen->bound_struct` via
   plain `->` -- same problem, one level further down the pointer chain.
   `janus_set_focus()`/`janus_toggle_box()` had the same issue reading
   `g_current_screen->bound_struct`.

Fixed by adding `janus_screen_load()` (mirrors the existing
`janus_widget_load()` -- a `memcpy_P` of the whole struct into a RAM
local before reading any field) and `janus_app_get_screen()` (a
`pgm_read_ptr`-based accessor for `janus_app_t.screens`) to
`janus_runtime.h`/`.c`, and switching every direct-index/direct-`->` call
site (in `janus_runtime.c`, `janus_input_focus.c`, and `main.cpp`) to use
them. Neither needed a `#ifdef __AVR__` at the call site --
`janus_runtime.h` already defines `pgm_read_ptr`/`memcpy_P` portably
(real macros on AVR, plain-memory fallbacks elsewhere), same pattern
`janus_widget_load()` already established.

Symptom before the fix, for anyone hitting this again: the boot sequence
never reached the print statement placed right after the first
`janus_render_screen()` call (even after 30-60s), while the panel kept
visibly changing -- a few widgets rendering plausibly at first (by luck,
reading nearby valid-ish flash), then one `fill_rect()` call's tiling
loop running away (traced `x` in `draw_area_sync` climbing past 27,000+
across 3000+ calls with `y`/tile size held constant -- a rect whose `.w`
was never a real authored value), eventually painting the whole panel
one flat color.

Also found and worked around, not fixed: `dac0.begin(0x62)`/
`dac1.begin(0x63)` (`MCP4725`/`I2CDevice`) hangs forever in `twi.c`'s
unbounded `while(TWI_..._state)` loops when the DAC doesn't ack --
unrelated to Janus, just hit in the same bring-up session. Both `.begin()`
calls and the two `dac*.setVoltage()` calls are commented out in
`main.cpp` as a temporary bypass, not a real fix -- next session should
either confirm the DAC wiring/address against hardware or add a timeout
around the TWI wait loops.

Verified on real hardware (Mega2560, `COM7`): `pwm_screen` (header + 2
boxes of mixed toggle/label/progress widgets) renders correctly after the
fix; a full blocking `janus_render_screen()` call for it measured
**~41-42ms**, comfortably inside the ~100ms periodic-redraw cadence
`main.cpp`'s loop uses for telemetry/bus-status refresh.

RAM/Flash dropped sharply from the pre-migration baseline (the old
`lib/Elements`/`lib/GUI` this replaces) -- **43.9% (3595/8192 B)** RAM,
**16.1% (40992/253952 B)** Flash, down from 75.8%/6213 B and 22%-ish at
the 2026-08-16 baseline. Most of Janus's generated widget/screen data is
`PROGMEM`-resident by design, unlike the old hand-written UI.
