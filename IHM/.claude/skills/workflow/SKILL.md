---
name: IHM-workflow
description: Governs how Claude plans, branches, implements, and tracks progress on the IHM project (github.com/rafaelchiafarelli/ArduinoIHM) and any work organized as initiatives/epics/tasks under an initiatives/ folder. Use this whenever the user is doing IHM planning or implementation work — creating or scoping initiatives, epics, or tasks; starting or naming a git branch for IHM work; implementing a single task file; deciding what counts as done; or figuring out how to hand off work between sessions. Also trigger this when the user mentions "initiative", "epic", "task file", "contract" (in the interface/schema sense), or references branches named feature/... or fixes/.... This is a process skill, not a code-generation skill — consult it even for small asks like "start the next task" or "is this task done yet", not just big planning requests.
---

# IHM Workflow

This skill encodes how Rafael works on IHM: a strict initiative → epic → task planning hierarchy, a matching git branch hierarchy, a narrow implementation loop per task, and a small number of hard stop-and-flag rules. The point of all of this is that Rafael runs multiple parallel sessions on separate repo copies. A session that doesn't follow this structure produces work another session (or Rafael) can't safely pick up. When in doubt, favor smaller, more self-contained, more explicitly-flagged over bigger and smoother.
...
- `main` — beta/release branch. Releases for testers and for programs built with IHM get cut from here. Treat it as something people outside this session depend on.
...
- **During planning**, whenever a decision would otherwise get made implicitly. This is the standing IHM principle: anything that should be an explicit `.yaml` DSL declaration must never be inferred by the generator or quietly decided while scoping a task.
