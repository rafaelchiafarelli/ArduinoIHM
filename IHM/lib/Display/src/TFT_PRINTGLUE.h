...
#include <Display_kbv.h> // Hardware-specific library
#if !defined(AVR)
...
class TFT_PRINTGLUE : public Display
{
    public:
        TFT_PRINTGLUE() {}
        void begin(void) {
        ...
        {
            Display::pushColors((uint8_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint8_t len)
        {
            Display::pushColors((uint16_t*)data, len, _first);
            _first = false;
        }

        void pushColors(uint16_t *data, uint16_t len, int first)
        {
            Display::pushColors((uint16_t*)data, len, first);
        }

        void pushColor(uint16_t color)
        {
            uint16_t c = color;
            Display::pushColors(&c, 1, _first);
            _first = false;
        }
        ...
        int16_t drawString(const char *string, int16_t poX, int16_t poY, uint8_t idx)
        {
            ...
            Display::setCursor(poX, poY + _ydatum);
            print(string);
            return w;
        }
