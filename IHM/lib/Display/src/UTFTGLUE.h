...
class UTFTGLUE : public Display
{
    public:
//  UTFTGLUE() : Display() {}
    UTFTGLUE(int model_ID, int RS, int WR,int CS, int RST, int RD = A0)
             : Display(CS, RS, WR, RD, RST) { _model_ID = model_ID; }
...
//         if (ID == 0x9327 && orientation == LANDSCAPE) orientation = 3;
        Display::begin(ID);
        Display::setRotation(_orient = orientation);
        _radius = 4;
    }
    void clrScr() { Display::fillScreen(0x0000);}
    void drawPixel(int x, int y) { Display::drawPixel(x, y, _fcolor);}
    void drawLine(int x1, int y1, int x2, int y2) { Display::drawLine(x1, y1, x2, y2, _fcolor);}
    void fillScr(uint16_t color) { Display::fillScreen(color);}
    void fillScr(byte r, byte g, byte b) { Display::fillScreen(setrgb(r, g, b));}
    void drawRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        Display::drawRect(x1, y1, w, h, _fcolor);
    }
    void drawRoundRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        Display::drawRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void fillRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        Display::fillRect(x1, y1, w, h, _fcolor);
    }
    void fillRoundRect(int x1, int y1, int x2, int y2) {
        int w = x2 - x1 + 1, h = y2 - y1 + 1;
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        Display::fillRoundRect(x1, y1, w, h, _radius, _fcolor);
    }
    void drawCircle(int x, int y, int radius) { Display::drawCircle(x, y, radius, _fcolor);}
    void fillCircle(int x, int y, int radius) { Display::fillCircle(x, y, radius, _fcolor);}
    void setColor(byte r, byte g, byte b)  { setColor(setrgb(r, g, b));}
    void setColor(uint16_t c)  { Display::setTextColor(_fcolor = c, _bcolor); }
    ...
    void setBackColor(uint32_t c)  { 
        ...
        Display::setTextColor(_fcolor, _bcolor);
    }
    void print(const char *st, int x, int y, int deg=0)  {
        settextcursor((char*)st, x, y); Display::print(st);}
    void print(char *st, int x, int y, int deg=0)  {
        settextcursor(st, x, y); Display::print(st);}
    void print(String st, int x, int y, int deg=0) {
        settextcursor((char*)(st.c_str()), x, y); Display::print(st);}
    ...
    void printNumF(double num, byte dec, int x, int y, char divider='.', int length=0, char filler=' ') {
        ...
        settextcursor(buf, x, y, length * _dig_wid * Display::textsize_x); 
        Display::print(buf);
    }
    void setTextSize(int sz) { Display::setTextSize(gfxFont == NULL ? sz : 1); } // ####### GFX ########
    ...
    void drawBitmap(int x, int y, int sx, int sy, const uint16_t *data, int scale=1) {
        uint16_t color;
        Display::setAddrWindow(x, y, x + sx*scale - 1, y + sy*scale - 1);
        if (scale == 1) Display::pushColors((const uint8_t*)data, sx * sy, 1);
        else {
...
//  void lcdOn();
    ...
    int  getDisplayXSize() { return Display::width(); }
    int  getDisplayYSize() { return Display::height(); }
...
//    uint8_t _ascend, _descend, _dig_wid;
    ...
    void settextcursor(char *st, int x, int y, int pad = 0) {
        ...
        if (is_gfx) {    //only GFX fonts need to paint background
            ...
            if (_fcolor != _bcolor) Display::fillRect(x, y, len + pad + 1, h, _bcolor);
            y += h;
        }
        Display::setCursor(x + pad, y);
    }
