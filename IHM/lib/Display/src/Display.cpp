...
Display::Display(int CS, int RS, int WR, int RD, int _RST):GFX(240, 320)
{
...
void Display::WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
...
void Display::pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
uint16_t Display::readReg(uint16_t reg, int8_t index)
{
    uint16_t ret;
    uint8_t lo;
    if (!done_reset)
...
uint32_t Display::readReg32(uint16_t reg)
{
...
uint32_t Display::readReg40(uint16_t reg)
{
    uint16_t h = readReg(reg, 0);
...
int16_t Display::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void Display::pushColors(uint16_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 0);
}
void Display::pushColors(uint8_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 2);   //regular bigend
}
void Display::pushColors(const uint8_t * block, int16_t n, bool first, bool bigend)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, bigend ? 3 : 1);
}

void Display::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void Display::begin(uint16_t ID)
{
...
//            0xB7, 1, 0x06,      //Entry Mode Set [06]
    ...
    case 0x9341:
      ...
      common_9341:	
        ...
        static const uint8_t ILI9341_regValues_ada[] PROGMEM = {        // TFTLCD only works with EXTC=0
