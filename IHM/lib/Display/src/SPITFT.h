...
#if defined(_PYPORTAL) || defined(_PYPORTAL_M4_TITANO) ||      \
    defined(_PYBADGE_M4_EXPRESS) ||                                    \
    defined(_PYGAMER_M4_EXPRESS) ||                                    \
    defined(_MONSTER_M4SK_EXPRESS) || defined(NRF52_SERIES) ||         \
    defined(_CIRCUITPLAYGROUND_M0)
...
// If DMA is enabled, Arduino sketch MUST #include <ZeroDMA.h>
// Estimated RAM usage:
...
/*!
  @brief  SPITFT is an intermediary class between GFX
...
public:
  ...
  SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi,
  ...
  SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc,
                  int8_t rst = -1);
  ...
  SPITFT(uint16_t w, uint16_t h, SPIClass *spiClass, int8_t cs,
  ...
  SPITFT(uint16_t w, uint16_t h, tftBusWidth busWidth, int8_t d0,
  ...
  */
  ...
  using GFX::drawRGBBitmap; // Check base class first
  ...
  // SPITFT_Macros.h. Some have been made into inline functions
