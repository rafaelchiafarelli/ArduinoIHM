...
typedef enum _BusIO_SPIRegType {
...
public:
  BusIO_Register(I2CDevice *i2cdevice, uint16_t reg_addr,
                          uint8_t width = 1, uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  BusIO_Register(SPIDevice *spidevice, uint16_t reg_addr,
                          BusIO_SPIRegType type, uint8_t width = 1,
                          uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  BusIO_Register(I2CDevice *i2cdevice,
                          SPIDevice *spidevice,
                          BusIO_SPIRegType type, uint16_t reg_addr,
                          uint8_t width = 1, uint8_t byteorder = LSBFIRST,
                          uint8_t address_width = 1);

  BusIO_Register(GenericDevice *genericdevice,
...
private:
  ...
  GenericDevice *_genericdevice;
  BusIO_SPIRegType _spiregtype;
  uint16_t _address;
...
public:
  BusIO_RegisterBits(BusIO_Register *reg, uint8_t bits,
                              uint8_t shift);
