...
*/
void GenericDevice::end(void) { _begun = false; }

/*! @brief Write a buffer of data
   @param buffer Pointer to buffer of data to write
   @param len Number of bytes to write
   @return true if write was successful, otherwise false */
bool GenericDevice::write(const uint8_t *buffer, size_t len) {
  if (!_begun)
    return false;
...
bool GenericDevice::read(uint8_t *buffer, size_t len) {
  if (!_begun)
    return false;
...
/*! @brief Read from a register location
   ...
   @return true if read was successful, otherwise false */
bool GenericDevice::readRegister(uint8_t *addr_buf, uint8_t addrsiz,
...
/*! @brief Write to a register location
   ...
   @return true if write was successful, otherwise false */
bool GenericDevice::writeRegister(uint8_t *addr_buf, uint8_t addrsiz,
                                           const uint8_t *buf,
                                           uint16_t bufsiz) {
