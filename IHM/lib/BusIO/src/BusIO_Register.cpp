 ...
 * to 1 byte
 */
BusIO_Register::BusIO_Register(I2CDevice *i2cdevice,
                                                 uint16_t reg_addr,
                                                 uint8_t width,
                                                 uint8_t byteorder,
 ...
 * to 1 byte
 */
BusIO_Register::BusIO_Register(SPIDevice *spidevice,
                                                 uint16_t reg_addr,
                                                 BusIO_SPIRegType type,
                                                 uint8_t width,
                                                 uint8_t byteorder,
                                                 uint8_t address_width) {
 ...
 * to 1 byte
 */
BusIO_Register::BusIO_Register(
    I2CDevice *i2cdevice, SPIDevice *spidevice,
    BusIO_SPIRegType type, uint16_t reg_addr, uint8_t width,
 ...
 */
BusIO_Register::BusIO_Register(
    GenericDevice *genericdevice, uint16_t reg_addr, uint8_t width,
    uint8_t byteorder, uint8_t address_width) {
  _i2cdevice = nullptr;
  _spidevice = nullptr;
 ...
 * uncheckable)
 */
bool BusIO_Register::write(uint8_t *buffer, uint8_t len) {
 ...
 * uncheckable)
 */
bool BusIO_Register::write(uint32_t value, uint8_t numbytes) {
  if (numbytes == 0) {
    numbytes = _width;
  }
  if (numbytes > 4) {
 ...
 *    @return Returns 0xFFFFFFFF on failure, value otherwise
 */
uint32_t BusIO_Register::readCached(void) { return _cached; }
...
*/
bool BusIO_Register::read(uint8_t *buffer, uint8_t len) {
  uint8_t addrbuffer[2] = {(uint8_t)(_address & 0xFF),
                           (uint8_t)(_address >> 8)};
  if (_i2cdevice) {
 ...
 * uncheckable)
 */
bool BusIO_Register::read(uint16_t *value) {
  if (!read(_buffer, 2)) {
    return false;
  }
 ...
 * uncheckable)
 */
bool BusIO_Register::read(uint8_t *value) {
  if (!read(_buffer, 1)) {
    return false;
  }
 ...
 */
void BusIO_Register::print(Stream *s) {
  uint32_t val = read();
  s->print("0x");
  s->print(val, HEX);
}
 ...
 *    @param  s The Stream to print to, defaults to &Serial
 */
void BusIO_Register::println(Stream *s) {
  print(s);
  s->println();
}
 ...
 * touching other bits
 *    @param  reg The BusIO_Register which defines the bus/register
 *    @param  bits The number of bits wide we are slicing
 *    @param  shift The number of bits that our bit-slice is shifted from LSB
 */
BusIO_RegisterBits::BusIO_RegisterBits(
    BusIO_Register *reg, uint8_t bits, uint8_t shift) {
  _register = reg;
  _bits = bits;
  _shift = shift;
}
 ...
 */
void BusIO_Register::setAddressWidth(uint16_t address_width) {
  _addrwidth = address_width;
}
