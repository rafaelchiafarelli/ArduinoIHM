 ...
 */
SPIDevice::SPIDevice(int8_t cspin, uint32_t freq,
 ...
 *    @param  dataMode The SPI mode to use, defaults to SPI_MODE0
 */
SPIDevice::SPIDevice(int8_t cspin, int8_t sckpin,
 ...
 */
void SPIDevice::transfer(uint8_t *buffer, size_t len) {
  //
  // HARDWARE SPI
  //
  if (_spi) {
 ...
 */
uint8_t SPIDevice::transfer(uint8_t send) {
  uint8_t data = send;
  transfer(&data, 1);
  return data;
}
 ...
 * SPI)
 */
void SPIDevice::beginTransaction(void) {
  if (_spi) {
#ifdef BUSIO_HAS_HW_SPI
 ...
 */
void SPIDevice::endTransaction(void) {
  if (_spi) {
#ifdef BUSIO_HAS_HW_SPI
    _spi->endTransaction();
#endif
 ...
 */
void SPIDevice::setChipSelect(int value) {
  if (_cs != -1) {
 ...
 */
void SPIDevice::beginTransactionWithAssertingCS() {
  beginTransaction();
  setChipSelect(LOW);
}
 ...
 */
void SPIDevice::endTransactionWithDeassertingCS() {
  setChipSelect(HIGH);
  endTransaction();
}
 ...
 * writes
 */
bool SPIDevice::write(const uint8_t *buffer, size_t len,
 ...
 * writes
 */
bool SPIDevice::read(uint8_t *buffer, size_t len, uint8_t sendvalue) {
 ...
 * writes
 */
bool SPIDevice::write_then_read(const uint8_t *write_buffer,
 ...
 * writes
 */
bool SPIDevice::write_and_read(uint8_t *buffer, size_t len) {
  beginTransactionWithAssertingCS();
  transfer(buffer, len);
  endTransactionWithDeassertingCS();
