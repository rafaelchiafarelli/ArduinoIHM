...
uint8_t SerialCommunication::send_one()
{
    uint8_t ret = 0;
...
bool SerialCommunication::Parse(uint16_t dest[], size_t max_size)
{
    uint8_t checksum = 0;
...
void SerialCommunication::send(const char *str, size_t s)
{
...
bool SerialCommunication::fast_handler(uint16_t dest[], size_t max_size)
{
...
bool SerialCommunication::decode(uint16_t dest[], size_t max_size)
{
...
int SerialCommunication::set(char *t, uint16_t dest[], int index)
{
    uint16_t tmp = atol(t);
    dest[index] = tmp;
    return 1;
