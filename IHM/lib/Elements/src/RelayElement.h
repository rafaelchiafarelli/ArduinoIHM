...
class RelayElement : public Element {
    private:
        ...
        void refreshDisplay() {
            ...
            status.setLabel(buf);
            led.setState(config.state ? LED_ON : LED_OFF);
        }

    public:
        RelayElement(uint16_t x, uint16_t y, uint16_t w, uint16_t h, Display *tft, Relay *relay, uint8_t relayIndex):
                                            Element(tft),
                                            led(x + RELAYELEMENT_LED_X, y + h / 2, RELAYELEMENT_LED_TEXTSIZE, RELAYELEMENT_LED_RADIUS, LED_OFF, "Rele", 0, tft),
                                            status(x + RELAYELEMENT_STATUS_X, y + h / 2 - 8, RELAYELEMENT_STATUS_TEXTSIZE, h, LABEL_ON, "Desl.", 1, tft),
