...
*/
GrayOLED::GrayOLED(uint8_t bpp, uint16_t w, uint16_t h,
                                     TwoWire *twi, int16_t rst_pin,
                                     uint32_t clkDuring, uint32_t clkAfter)
    : GFX(w, h), i2c_preclk(clkDuring), i2c_postclk(clkAfter),
...
GrayOLED::GrayOLED(uint8_t bpp, uint16_t w, uint16_t h,
                                     ...
                                     int16_t cs_pin)
    : GFX(w, h), dcPin(dc_pin), csPin(cs_pin), rstPin(rst_pin),
      _bpp(bpp) {

  spi_dev = new SPIDevice(cs_pin, sclk_pin, -1, mosi_pin, 1000000);
}
...
GrayOLED::GrayOLED(uint8_t bpp, uint16_t w, uint16_t h,
  ...
  spi_dev = new SPIDevice(cs_pin, bitrate, SPI_BITORDER_MSBFIRST,
                                   SPI_MODE0, spi);
}

/*!
    @brief  Destructor for GrayOLED object.
*/
GrayOLED::~GrayOLED(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
*/
void GrayOLED::oled_command(uint8_t c) {
  if (i2c_dev) {                // I2C
...
bool GrayOLED::oled_commandList(const uint8_t *c, uint8_t n) {
  if (i2c_dev) {            // I2C
...
*/
bool GrayOLED::_init(uint8_t addr, bool reset) {
  ...
  if (_theWire) { // using I2C
    i2c_dev = new I2CDevice(addr, _theWire);
...
/*!
    @brief  Set/clear/invert a single pixel. This is also invoked by the
            GFX library in generating many higher-level graphics
            primitives.
    @param  x
...
*/
void GrayOLED::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void GrayOLED::clearDisplay(void) {
...
*/
bool GrayOLED::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch (getRotation()) {
    case 1:
...
*/
uint8_t *GrayOLED::getBuffer(void) { return buffer; }
...
*/
void GrayOLED::invertDisplay(bool i) {
...
*/
void GrayOLED::setContrast(uint8_t level) {
