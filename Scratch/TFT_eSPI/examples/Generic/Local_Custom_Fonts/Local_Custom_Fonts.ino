/*
  ...
  This example shows the use of a GFX custom font with a
  ...
  Note that online converters for GFX compatible fonts are
...
void loop() {
  ...
  // Unlike the stock GFX library, the TFT_eSPI library DOES optionally draw
  ...
  tft.fillScreen(TFT_BLUE);  // Clear screen
  tft.setCursor(0,60);       // To be compatible with GFX the cursor datum is always bottom left
